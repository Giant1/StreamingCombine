...
class CombinerHourly2 {

}

object CombinerHourly2 {
  final val SEPERATOR = "\t"
  ...
  def parseMessage(message: String): RptEffectBase = {
    ...
    base.setVideoViewType(array(19))
  ...
  def aggregation(rdd: RDD[String]): RDD[(String, String)] = {
    val reduceRDD = rdd
      .map { line =>
        ...
        val base: RptEffectBase = parseMessage(line)
        val adSlotTypeAndVideoViewType = base.getAdSlotType + "\t" + base.getVideoViewType
        ((base.getPartnerId, base.getAdvertiserCompanyId, base.getAdvertiserId, base.getOrderId, base.getCampaignId,
          ...
          base.getCreativeId, base.getCreativeType, base.getInventoryType, adSlotTypeAndVideoViewType, base.getPlatform,
          base.getDay, base.getClientId, base.getCampaignDivisionId, base.getSubCampaignDivisionId, base.getExeCampaignDivisionId, base.getSubPlatform , requestDay + hour
          ),
      ...
      conversionPixel, creativeSize, creativeId, creativeType, inventoryType, adSlotTypeAndVideoViewType, platform,
...
//      logger.info(s"--------------- $adSlotTypeAndVideoViewType -------------")

      val typeArr = adSlotTypeAndVideoViewType.split("\t", -1)
      var adSlotType = "null"
      var videoViewType = "na"
      if(typeArr.length == 2) {
        adSlotType = if (typeArr(0).isEmpty || typeArr(0) == "") "null" else typeArr(0)
        videoViewType = if (typeArr(1).isEmpty || typeArr(1) == "") "na" else typeArr(1)
      }
      ...
      val result = s"$partnerId\t$advertiserCompanyId\t$advertiserId\t$orderId\t$campaignId\t$subCampaignId\t$exeCampaignId\t$verticalTagId" +
        ...
        s"\t$client_id\t$campaign_division_id\t$sub_campaign_division_id\t$exe_campaign_division_id\t$f_sub_platform\t$videoViewType"
  ...
  def aggregation(rdd: DStream[String]): DStream[(String, String)] = {
    val reduceRDD = rdd
      .map { line =>
        ...
        ((base.getPartnerId, base.getAdvertiserCompanyId, base.getAdvertiserId, base.getOrderId, base.getCampaignId,
          base.getSubCampaignId, base.getExeCampaignId, base.getVerticalTagId, base.getConversionPixel, base.getCreativeSize,
          base.getCreativeId, base.getCreativeType, base.getInventoryType, base.getAdSlotType, base.getPlatform, base.getDay, requestDay + hour,
...
//      println("event._1: " + event._1.toString + " ||| event._2: " + event._2.toString + "^^^ length:" + event._2.split("\t").length)
  ...
  def processRDD(rdd: RDD[(String, String, String)])(implicit sqlContext: HiveContext): Unit = {
    ...
    val schemaString = (1 to 36).map(i => "C" + i).mkString("\t")
...
//    println("schema:toString" + schema.toString())
    writeDataByTable(normalRDD, config.getString("spark.day_table"), schema, sqlContext)
